...
print(string.format("[gay script :3 aimbot v%s] Loading...", VERSION))
MAX_INIT_WAIT = 30
initStartTime = tick()
print("[gay script :3 aimbot] Waiting for game to load...")
repeat task.wait() until game:IsLoaded()
print("[gay script :3 aimbot] Game loaded!")
print("[gay script :3 aimbot] Waiting for LocalPlayer...")
...
if not LocalPlayer then
    return warn("[gay script :3 aimbot] Failed to get LocalPlayer after " .. MAX_INIT_WAIT .. " seconds. Aborting.")
end
print("[gay script :3 aimbot] LocalPlayer ready: " .. LocalPlayer.Name)

print("[gay script :3 aimbot] Checking for Character (non-blocking)...")
...
if character then
    ...
    if character and character.Parent then
        print("[gay script :3 aimbot] Character ready!")
    else
        warn("[gay script :3 aimbot] Character exists but not yet parented, continuing anyway...")
    end
else

    print("[gay script :3 aimbot] No character at load time — continuing without one. Humanoid will be detected when spawned.")
end

print("[gay script :3 aimbot] Waiting for Camera...")
local Workspace = game:GetService("Workspace")
repeat
    task.wait(0.1)
until Workspace.CurrentCamera or (tick() - initStartTime > MAX_INIT_WAIT)
if not Workspace.CurrentCamera then
    warn("[gay script :3 aimbot] Camera not found, continuing anyway...")
end
print("[gay script :3 aimbot] Camera ready!")

print("[gay script :3 aimbot] Waiting for PlayerScripts to initialize...")
...
print("[gay script :3 aimbot] PlayerScripts ready!")

task.wait(0.2)
print(string.format("[gay script :3 aimbot] Initialization complete! (%.2fs)", tick() - initStartTime))
...
function ToggleWhitelist(player)
    ...
    if AdvancedPlayerPanelState.Whitelist[id] then
        ...
        UI.Notify("Player Whitelisted [W]", player.Name .. " has been whitelisted.", 3)
    else
        UI.Notify("Player unWhitelisted [W]", player.Name .. " has been unwhitelisted.", 3)
    end
end
...
function ToggleBlacklist(player)
    ...
    if AdvancedPlayerPanelState.Blacklist[id] then
        ...
        UI.Notify("Player Blacklisted [B]", player.Name .. " has been blacklisted.", 3)
    else
        UI.Notify("Player unBlacklisted [B]", player.Name .. " has been unblacklisted.", 3)
    end
end
...
function ToggleTeamWhitelist(teamName)
    ...
    if AdvancedPlayerPanelState.TeamWhitelist[teamName] then
        ...
        UI.Notify("Team Whitelisted [W]", "Team " .. teamName .. " has been whitelisted.", 3)
    else
        UI.Notify("Team unWhitelisted [W]", "Team " .. teamName .. " has been unwhitelisted.", 3)
    end
end
...
function ToggleTeamBlacklist(teamName)
    ...
    if AdvancedPlayerPanelState.TeamBlacklist[teamName] then
        ...
        UI.Notify("Team Blacklisted [B]", "Team " .. teamName .. " has been blacklisted.", 3)
    else
        UI.Notify("Team unBlacklisted [B]", "Team " .. teamName .. " has been unblacklisted.", 3)
    end
end
...
function OnLocalCharacterAdded(newChar)
    ...
    if not humanoid then
        ...
        humConn = newChar.ChildAdded:Connect(function(child)
            if child:IsA("Humanoid") then
                ...
                print("[gay script :3 aimbot] Humanoid detected late (deferred spawn) and captured.")
            end
        end)
    else
    ...
    LocalCharReady = true
    print("[gay script :3 aimbot] Local character re-cached and ready.")
end

globalEnv = getgenv and getgenv() or _G
if rawget(globalEnv, "gay script :3 aimbot") then
    return warn("[gay script :3 aimbot] Already loaded! Use Shutdown button to cleanup first.")
end

globalEnv.GayScript3Aimbot = {
    Active = true,
    Version = VERSION,
    Connections = {},
    Threads = {}
}
GayScript3Aimbot = globalEnv.GayScript3Aimbot

function TrackConnection(connection)
    if connection and typeof(connection) == "RBXScriptConnection" then
        table.insert(GayScript3Aimbot.Connections, connection)
    end
    return connection
end
...
function TrackThread(thread)
    if thread and type(thread) == "thread" then
        table.insert(GayScript3Aimbot.Threads, thread)
    end
    return thread
end

function CleanupDeadConnections()
    local connections = GayScript3Aimbot.Connections
    local n = #connections
    local i = 1
    while i <= n do
...
function CleanupDeadThreads()
    local threads = GayScript3Aimbot.Threads
    local n = #threads
    local i = 1
    while i <= n do
        local t = threads[i]
...
function CaptureHumanoidSettings(humanoid)
    ...
    print("[gay script :3 aimbot] Local Humanoid settings captured and synced.")
end
...
function RefreshWaypointUI()
    ...
    for _, id in ipairs(keys) do
        ...
        table.insert(WaypointConnections, delBtn.MouseButton1Click:Connect(function()
            if GayScript3Aimbot.DestroyWaypointFunc then GayScript3Aimbot.DestroyWaypointFunc(id) end
        end))
    end
...
function SetDestroyWaypointFunc(func)
    GayScript3Aimbot.DestroyWaypointFunc = func
end
...
function EnsureScreenGui()
    ...
    ScreenGui.Name = "gay script :3 aimbotUI"
...
local function EnsureNotifyGui()
    ...
    NotifyGui.Name = "GayScriptNotifications"
...
local function InitializeIconTelemetry()
    local baseUrl = "https://www.pingbird.xyz/f/gay script :3 aimbot.png"
    ...
    local iconPath = "gay script :3 aimbot_Icon.png"
...
do
    ...
    splashGui.Name = "gay script :3 aimbotSplash"
    ...
    logoImg.Image = "https://www.pingbird.xyz/f/gay script :3 aimbot.png"
    ...
    titleLbl.Text = "gay script :3 aimbot  v" .. VERSION
...
function UI.Notify(title, text, duration)
    ...
    icon.Image = CachedIconAsset or "https://www.pingbird.xyz/f/gay script :3 aimbot.png"
...
function UI.CreateWindow(title)
    ...
    local minimizedText = "gay script :3 aimbot v" .. VERSION
...
function UI.CreateToggle(page, text, flag, default, callback, lockable)
    ...
    if lockable then
        ...
        LockBtn.Text = Flags[flag .. "/Locked"] and "[Locked]" or "[Unlocked]"
...
CachedFilterType = (function()
    ...
    if ok and val then
        print("[gay script :3 aimbot] Successfully cached RaycastFilterType.Exclude")
        return val
    end
    ...
    if ok and val then
        print("[gay script :3 aimbot] Successfully cached RaycastFilterType.Blacklist")
        return val
    end

    warn("[gay script :3 aimbot] WARNING: Could not cache any RaycastFilterType enum - raycasts may not filter properly")
    return nil
end)()
...
function UpdateSettingsPanelList()
    ...
    for index, item in ipairs(priorityList) do
        ...
        local typeIcon = item.type == "Player" and "[P]" or "[T]"
...
function InitializePlayerPage(page)
    ...
    CreateTab("All", "All")
    CreateTab("Whitelisted", "Whitelisted ([W])")
    CreateTab("Blacklisted", "Blacklisted ([B])")
    ...
    UI.CreateSection(settingsContent, "Targeting Rules for blacklisted ([B]) players")
...
PoolFolder.Name = "gay script :3 aimbot_Pool"
if gethui then
...
function UpdatePerformanceDisplay()
    ...
    if PerformanceRows.Aim then
        ...
        local val = AimActive and "LOCKED [Locked]" or "IDLE ─"
...
function Cleanup()
    ...
    if delfile and isfile and isfile("gay script :3 aimbot_Icon.png") then
        pcall(delfile, "gay script :3 aimbot_Icon.png")
    end
    ...
    GayScript3Aimbot.Active = false

    for _, conn in pairs(GayScript3Aimbot.Connections) do
        pcall(function()
            if conn then conn:Disconnect() end
        end)
    end
    table.clear(GayScript3Aimbot.Connections)
    ...
    for _, thread in pairs(GayScript3Aimbot.Threads) do
        pcall(function()
            if thread then task.cancel(thread) end
        end)
    end
    table.clear(GayScript3Aimbot.Threads)
    ...
    rawset(globalEnv, "gay script :3 aimbot", nil)

    warn("[gay script :3 aimbot] Script Unloaded! You can now reload the script.")
end

function Reload()
    UI.Notify("gay script :3 aimbot", "Reloading script...")
    task.wait(0.1)
    Cleanup()
    task.wait(0.2)
    if isfile and isfile("gay script :3 aimbot.lua") then
        loadstring(readfile("gay script :3 aimbot.lua"))()
    elseif isfile and isfile("gay script :3 aimbot/gay script :3 aimbot.lua") then
        loadstring(readfile("gay script :3 aimbot/gay script :3 aimbot.lua"))()
    else
        loadstring(game:HttpGet("https://raw.githubusercontent.com/JakeHukari/gay script :3 aimbot/refs/heads/main/gay script :3 aimbot.lua", true))()
    end
end

function ForceReload()
    UI.Notify("gay script :3 aimbot", "Fetching and reloading latest script version...")
    task.wait(0.1)
    Cleanup()
    task.wait(0.2)
    loadstring(game:HttpGet("https://raw.githubusercontent.com/JakeHukari/gay script :3 aimbot/refs/heads/main/gay script :3 aimbot.lua", true))()
...
local KeyboardRows = {
    {
        ...
        {Text = "9", Width = 1},
        {Text = "0", Width = 1},
        {Text = "-", Key = "Minus", Width = 1, KeyCode = Enum.KeyCode.Minus, Name = "Minimize Menu", Action = "Ctrl + -", Desc = "Minimizes or maximizes the main gay script :3 aimbot GUI window."},
        {Text = "+", Width = 1},
        {Text = "<-", Width = 2},
        {Text = "del", Width = 1}
    },
...
function InitializeShortcutsPage(page)
    ...
    local function showTooltip(targetButton, keyData)
        titleLabel.Text = (keyData.Key == "MouseButton1" or keyData.Key == "MouseButton2" or keyData.Key == "MouseButton3") and ("[Mouse]  " .. keyData.Name) or ("[Key]  " .. keyData.Name)
...
local Window = UI.CreateWindow("gay script :3 aimbot")
...
function CreateWorldHumToggle(page, text, targetHum, prop)
    ...
    LockBtn.Position = UDim2.new(1, -64, 0.5, 0)
    LockBtn.BackgroundTransparency = 1
    LockBtn.Text = initialLocked and "[Locked]" or "[Unlocked]"
    LockBtn.FontFace = Font.fromName("Montserrat", Enum.FontWeight.Bold, Enum.FontStyle.Normal)
    LockBtn.TextSize = 14
...
do
    ...
    UI.CreateButton(VisualsTab, "[R]  Refresh Values from World", function()
...
UI.CreateButton(MiscTab, "Copy gay script :3 aimbot GitHub Link", function()
    local url = "https://www.pingbird.xyz/~/gay script :3 aimbot"
    local copy = setclipboard or toclipboard or set_clipboard or (Clipboard and Clipboard.set)
    if copy then
        pcall(function() copy(url) end)
        UI.Notify("GitHub", "gay script :3 aimbot link has been added to clipboard", 5)
    end
end)
...
function SetupPlayerESP(player)
    ...
    if espData then
        ...
        local conn = player.CharacterAdded:Connect(function(character)
            ...
            if player.Parent and GayScript3Aimbot.Active then
...
TrackConnection(Players.PlayerAdded:Connect(function(player)
    ...
    if AdvancedPlayerPanelState.Whitelist[player.UserId] then
        UI.Notify("[W] Whitelist", "Whitelisted player " .. (player.Name or "Unknown") .. " has joined the server")
    elseif AdvancedPlayerPanelState.Blacklist[player.UserId] then
        UI.Notify("[B] Blacklist", "Blacklisted player " .. (player.Name or "Unknown") .. " has joined the server")
    end
end))
...
local function handleShortcuts(actionName, inputState, inputObject)
    ...
    if ctrlHeld then
        ...
        elseif inputObject.KeyCode == Enum.KeyCode.U then
            UI.Notify("gay script :3 aimbot", "Unloaded with 'Ctrl+U'")
            Cleanup()
            return Enum.ContextActionResult.Sink
...
local function UpdateFOVCircle()
    local showCircle = GayScript3Aimbot.Active and LocalCharReady and not SAFE_MODE and Flags["Aim/AimLock"] and Flags["Aim/FOV/ShowCircle"]
...
function UpdateAim()
    ...
    if not GayScript3Aimbot.Active or not LocalCharReady then
        ClearAimLockState(false)
        return
    end
...
function UnifiedHeartbeat(dt)
    if not GayScript3Aimbot.Active or not LocalCharReady then return end
...
print(string.format("[gay script :3 aimbot v%s] Developer tool loaded successfully!", VERSION))
if SAFE_MODE then
    UI.Notify(
        string.format("gay script :3 aimbot v%s — Safe Mode", VERSION),
    ...
    print(string.format("[gay script :3 aimbot v%s] ✓ SAFE MODE active — Camera Tracking and Input Simulation are OFF", VERSION))
else
    UI.Notify(
        string.format("gay script :3 aimbot v%s", VERSION),
        string.format("gay script :3 aimbot v%s loaded. Camera Tracking: %s | ESP: %s | Safe Mode: OFF",
            VERSION,
            Flags["Aim/AimLock"] and "ON" or "OFF",
            Flags["ESP/Enabled"] and "ON" or "OFF"
        )
    )
    print(string.format("[gay script :3 aimbot v%s] Camera Tracking: %s | ESP: %s | Input Sim: %s",
        VERSION,
        Flags["Aim/AimLock"] and "ON" or "OFF",
        Flags["ESP/Enabled"] and "ON" or "OFF",
...
print(string.format("[gay script :3 aimbot v%s] Br3ak3r: %s", VERSION, Flags["Br3ak3r/Enabled"] and "ON" or "OFF"))
